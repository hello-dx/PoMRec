...
class BaseRunner(object):
    ...
    def exp(self, dataset: BaseModel.Dataset) -> np.ndarray:
        """
        Evaluate the results for an input dataset.
        :return: result dict (key: metric@k)
        """
        ...
        for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
            prediction = dataset.model(utils.batch_to_gpu(batch, dataset.model.device))['prediction']
            means.extend(dataset.model.interest_extractor.mean.cpu().data.numpy())
            stds.extend(dataset.model.interest_extractor.std.cpu().data.numpy())
            attn.extend(dataset.model.interest_extractor.attn.cpu().data.numpy())
            history.extend(dataset.model.interest_extractor.history.cpu().data.numpy())
        means = np.array(means)
        stds = np.array(stds)
        attn = np.array(attn)
        history = np.array(history)
        return means, stds, history, attn
